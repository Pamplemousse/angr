...
class ReachingDefinitionsState():
    ...
    def add_use(self, atom, code_loc):
        ...
        self.analysis.used_definitions_for_current_codeloc.update(self.get_definitions(atom))
    ...
    def _cycle(self, code_loc):
        if code_loc != self.analysis.current_codeloc:
            ...
            self.analysis.used_definitions_for_current_codeloc = set()

    def kill_and_add_definition(self, atom, code_loc, data, dummy=False):
        self._cycle(code_loc)

        # TODO: ReachingDefinitions state kill_and_add_definition

        if definition is not None:
            self.analysis.def_use_graph.add_node(definition)
            for used in self.analysis.used_definitions_for_current_codeloc:
    ...
    def add_use(self, atom, code_loc):
        ...
        # TODO: ReachingDefinitions state add_use
